...
class Files {
    ...
    static void setLastModifiedNow(File file) throws IOException {
        if (file.exists()) {
            ...
            DLogger.d("set file " + file.getName() + " modify time，modified？" + modified);
            if (!modified) {
                modify(file);
                if (file.lastModified() < now) {
                    ...
                    DLogger.d("Last modified date " + new Date(file.lastModified()) + " is not set for file " + file.getAbsolutePath());
                }
            }
        }
    }
